...
class Actuator:
    ...
    def set_last_prevision(self, forecast_list):
        ...
        try:
            for item in forecast_list:
                ...
                prevision = round(forecast['yhat'].head(1).values[0],
                                  2)  # valore dell'ultima previsione per lo specifico parametro
            ...
            print('Problem on setting last provision!')
    ...
    def actuate(self, acquisition_point):
        ...
        if self.gt is not None and self.gt <= 10 and self.at is not None and self.at <= 15:
            if roof != all_open:
                roof = all_open
                r = util_api.post_roof_and_irrigation(acquisition_point, roof, irrigation)
                if r == 'ok':
                    print('Roof set to all_open')
        ...
        if self.wf is not None:
            roof = random.randint(0, 100)
            irrigation = random.randint(0, 100)
            r = util_api.post_roof_and_irrigation(acquisition_point, roof, irrigation)
            util_dashboard.post_roof_and_irrigation(roof, irrigation)

        if self.wf is not None and self.wf <= 3:
            if irrigation != all_open:
                irrigation = all_open
                r = util_api.post_roof_and_irrigation(acquisition_point, roof, irrigation)
                if r == 'ok':
                    print('Irrigation set to all_open')
                util_dashboard.post_roof_and_irrigation(roof, irrigation)
        elif self.wf is not None and self.wf >= 5:
            if irrigation != all_close:
                irrigation = all_close
                r = util_api.post_roof_and_irrigation(acquisition_point, roof, irrigation)
                if r == 'ok':
                    print('Irrigation set to all_close')
                util_dashboard.post_roof_and_irrigation(roof, irrigation)
        ...
        if self.gh is not None and self.gh <= 70 and self.ah is not None and self.ah <= 30:
            ...
            if irrigation <= half_open:
                ...
                r = util_api.post_roof_and_irrigation(acquisition_point, roof, irrigation)
                if r == 'ok':
                    print('Irrigation set to half_open')
                util_dashboard.post_roof_and_irrigation(roof, irrigation)
